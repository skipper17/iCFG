...
def main(args):
    #path decision
    path = "/home/data/iclr24out/middle9_noun_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    for id in tqdm(range(1250)):
        ...
        half_text_embeddings = text_embeddings.clone()
        for pos in noun_tuples:
            half_text_embeddings[pos] = half_text_embeddings[pos] * 0.9 + uncond_embeddings[pos] * 0.1
        ...
        for t in tqdm(scheduler.timesteps):
            ...
            noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond) + alpha * 2 * (9 * noise_pred_text + noise_pred_uncond - 10 * half_noise_pred_text)
