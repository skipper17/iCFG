...
def main(args):
    #path decision
    path = "/home/data/iclr24out/init_embedding_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    for id in tqdm(range(1250)):
        ...
        with torch.no_grad():
            text_encoder.change_enhance(None)
            text_embeddings = text_encoder(text_input.input_ids.to(torch_device))
            text_embeddings = text_embeddings[0]
            text_encoder.change_enhance([0.5,noun_tuples])
            half_text_embeddings = text_encoder(text_input.input_ids.to(torch_device))
