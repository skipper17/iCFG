...
def main(args):
    #path decision
    path = "/home/data/iclr24out/middle9_all_ada_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    for id in tqdm(range(1250)):
        ...
        half_text_embeddings = text_embeddings * 0.9 + uncond_embeddings * 0.1
        ...
        for t in tqdm(scheduler.timesteps):
            ...
            noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond) + alpha * ALPHA_BASE * (COEF_1 * noise_pred_text + COEF_0 * noise_pred_uncond + COEF_MIDDLE * half_noise_pred_text)
            noise_org_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond) 
            noise_pred = block_adaIN(noise_org_pred, noise_pred, blocknum=32)
