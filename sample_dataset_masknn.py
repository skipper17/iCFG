...
def main(args):
    ...
    path = "tanhao_masknn_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    for id in tqdm(range(1250)):
        ...
        half_prompt = [" ".join(PADDING_TOKEN if text[1] == 'NN' or text[1] == 'NNS' or text[1] == 'NNPS' or text[1] == 'NNP' else text[0] for text in nltk.pos_tag(it.replace(".","").split())) for it in prompt]
