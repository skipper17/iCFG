...
def main(args):
    #path decision
    path = "/home/data/iclr24out/half_noun_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    for id in tqdm(range(1250)):
        ...
        for pos in noun_tuples:
            half_text_embeddings[pos] = (half_text_embeddings[pos] + uncond_embeddings[pos])/2
