...
def main(args):
    #path decision
    path = "/home/data/iclr24out/atten_v_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    unet2 = UNet2DConditionModel.from_pretrained(
        "runwayml/stable-diffusion-v1-5", subfolder="unet", use_safetensors=True, weaken_cross_attention = 0.5
    )
    ...
    print(SUM//batch_size//2)
    # sample
    for id in tqdm(range(SUM//batch_size)):

        # load from dataset
        begin = id * batch_size if id < SUM//batch_size//2 else (id - SUM//batch_size//2) * batch_size
        prompt = []
        ...
        all_embeddings = torch.cat([uncond_embeddings, text_embeddings])
        ...
        for t in tqdm(scheduler.timesteps):
            ...
            latent_model_input_middle = latents
            latent_model_input_middle = scheduler.scale_model_input(latent_model_input_middle, timestep=t)
            # predict the noise residual
            with torch.no_grad():
                noise_pred = unet(latent_model_input, t, encoder_hidden_states=all_embeddings).sample
                norse_pred_middle = unet2(latent_model_input_middle, t, encoder_hidden_states=text_embeddings).sample

            # perform guidance
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            half_noise_pred_text = norse_pred_middle
