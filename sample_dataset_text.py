...
def main(args):
    #path decision
    path = "text_guidance_scale_" + str(args.guidance_scale) + "_alpha_" + str(args.alpha) + "_" + str(time.time())[0:10]
    os.mkdir(path)
    ...
    for id in tqdm(range(1250)):
        ...
        half_prompt = [" ".join(it.split(" ")[:len(it.split(" "))//2]) for it in prompt]
